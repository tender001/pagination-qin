...
describe('Default Pagination', () => {
  ...
  it('onChange should be forbidden when total is default', () => {
    const pagers = wrapper.find('.pagination-qin-item');
...
describe('Uncontrolled Pagination', () => {
  ...
  function shouldHighlightRight() {
    const pagers = wrapper.find('li:not(.pagination-qin-total-text)');
    pagers.forEach((pager, index) => {
      // page starts from 1
      if (index === wrapper.state().current) {
        expect(pager.hasClass('pagination-qin-item-active')).toBe(true);
      } else {
        expect(pager.hasClass('pagination-qin-item-active')).toBe(false);
      }
    });
  }
  ...
  it('prev-button should be disabled', () => {
    const prevButton = wrapper.find('.pagination-qin-prev');
    expect(prevButton.hasClass('pagination-qin-disabled')).toBe(true);
  ...
  it('should calc page right', () => {
    const pagers = wrapper.find(
      'li:not(.pagination-qin-total-text):not(.pagination-qin-options)',
    );
  ...
  it('next button should not be disabled', () => {
    const nextButton = wrapper.find('.pagination-qin-next');
    expect(nextButton.hasClass('pagination-qin-disabled')).toBe(false);
  ...
  it('should response mouse click right', () => {
    ...
    expect(page2.hasClass('pagination-qin-item-2')).toBe(true);
  ...
  it('should quick jump to expect page', () => {
    const quickJumper = wrapper.find('.pagination-qin-options-quick-jumper');
    const input = quickJumper.find('input');
    const goButton = quickJumper.find(
      '.pagination-qin-options-quick-jumper-button',
    );
  ...
  it('should not jump when blur input when there is not goButton', () => {
    ...
    const quickJumper = component.find('.pagination-qin-options-quick-jumper');
    const input = quickJumper.find('input');
    input.simulate('change', { target: { value: '2' } });
    input.simulate('blur');
  ...
  it('should display total items', () => {
    const totalText = wrapper.find('.pagination-qin-total-text');
...
describe('Other props', () => {
  ...
  describe('showPrevNextJumpers props', () => {
    it('should hide jump-prev, jump-next if showPrevNextJumpers equals false', () => {
      ...
      const prev = wrapper.find('.pagination-qin-jump-prev');
      const next = wrapper.find('.pagination-qin-jump-next');
  ...
  describe('hideOnSinglePage props', () => {
    ...
    it('should hide pager if hideOnSinglePage equals true', () => {
      ...
      expect(wrapper.find('.pagination-qin').exists()).toBe(false);
    });
    ...
    it('should show pager if hideOnSinglePage equals false', () => {
      ...
      expect(wrapper.find('.pagination-qin').exists()).toBe(true);
    });
...
describe('current value on onShowSizeChange when total is 0', () => {
  ...
  it('should call onShowSizeChange when no change', () => {
    const sizeChanger = wrapper
      .find('.pagination-qin-options-size-changer')
      .at(0);
  ...
  it('when total is 0, pager should show `1` and being disabled', () => {
    const itemButton = wrapper.find('.pagination-qin-item');
    expect(itemButton.hasClass('pagination-qin-item-disabled')).toBe(true);
  ...
  it('size changer show logic', () => {
    ...
    expect(wrapper1.exists('.pagination-qin-options-size-changer')).toBe(false);
    const wrapper2 = mount(
      <Pagination selectComponentClass={Select} total={51} />,
    );
    expect(wrapper2.exists('.pagination-qin-options-size-changer')).toBe(true);
    ...
    expect(wrapper3.exists('.pagination-qin-options-size-changer')).toBe(false);
    const wrapper4 = mount(
      <Pagination selectComponentClass={Select} showSizeChanger total={50} />,
    );
    expect(wrapper4.exists('.pagination-qin-options-size-changer')).toBe(true);
  });
...
describe('should emit onChange when total is string', () => {
  ...
  it('onChange should be called when click page', () => {
    const pagers = wrapper.find('.pagination-qin-item-3');
    const page1 = pagers.at(0);
    page1.simulate('click');
